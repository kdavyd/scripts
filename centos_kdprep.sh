...
yum install -y pssh atop mosh cloud-utils-growpart python-pip wget tmux nano perf lsscsi screen expect expect-devel papi-devel gcc byacc flex
...
cd && git clone https://github.com/FeCastle/tiptop.git && cd tiptop && ./configure && make && make install clean && cd
